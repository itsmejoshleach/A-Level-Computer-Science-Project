...
if Current_Item_Name != '': # if a Item has been provided, add it, else finish item section
    ...
    Current_Item_Price = int(input(f"What is the price per item? (In {Currency}): "))
    Current_Item_Tax = int(input("Tax % (Leave empty if not tax): "))
...
try:
    ...
    pdf.gen(f"Invoice-{Current_Date}.pdf", generate_qr_code=True) # generate invoice (No Provider Name)